...
class DataGenerator:
    ...
    def all_sequential_patches(self, step_size):
        ...
        if self.no_weights:
            ...
            labels = y[..., [0]]
            weights = y[..., [1]]
            weights = np.where(weights >= self.wt_thr, 10, 1)
    ...
    def random_generator(self, BATCH_SIZE):
        ...
        Args:
            ...
            independently)
            wt_thr (int || float) (optional): Weight threshold to use for binarizing
        ...
        while True:
            ...
            if self.augmenter == "iaa":
                ...
                # weighting schemes.
                assert (
                    self.wt_thr or self.no_weights
                ), "Must use a weight threshold or no weights when using image augmentation."
